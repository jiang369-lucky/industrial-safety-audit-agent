...
CONFIG = {
    ...
    "check_interval_min": 1.0,  # 新增：最小巡检间隔（火花强时）
    "check_interval_max": 3.0,  # 新增：最大巡检间隔（火花弱时）
    "cache_duration": 2.0,
    ...
    # ==================== 输出配置 ====================
    "output_folder": r"D:\yolo\Spark-Gated MoE Framework\detection_results\video\trigger_enhanced_dynamic",
    # 输出文件夹路径，保存处理后的视频文件
...
class SparkIntensityDetector:
    """检测电焊火花的强度，用于动态调整Qwen调用间隔"""

    def __init__(self, config):
        self.config = config
        self.prev_frame_gray = None
        self.spark_history = deque(maxlen=30)  # 保存最近30帧的火花强度
        self.last_update_time = 0
        self.current_intensity = 0.0  # 0-1之间的强度值

    def detect_spark_intensity(self, frame):
        """
        检测火花强度
        返回: 0-1之间的强度值，0表示无火花，1表示最强火花
        """
        # 转换为灰度图
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        ...
        # 1. 计算帧间差异（捕捉瞬间亮度变化，火花的主要特征）
        frame_diff = cv2.absdiff(gray, self.prev_frame_gray)

        # 2. 计算亮度变化阈值（火花通常会产生高亮度变化区域）
        ...
        # 4. 检测高亮度区域（火花通常非常亮）
        high_brightness_mask = gray > 200  # 亮度超过200的像素
        ...
        grad_mean = np.mean(grad_magnitude) / 255.0  # 归一化

        # 综合火花强度（加权融合）
        # 帧间变化和高亮度区域是最重要的特征
        spark_intensity = (
                0.4 * min(1.0, diff_ratio * 10) +  # 帧间变化，放大10倍
                0.3 * min(1.0, high_brightness_ratio * 20) +  # 高亮度区域，放大20倍
                0.2 * min(1.0, edge_ratio * 5) +  # 边缘强度
                0.1 * min(1.0, grad_mean * 2)  # 梯度幅度
        )
    ...
    def get_dynamic_interval(self):
        ...
        if intensity > 0.6:
            return CONFIG["check_interval_min"]  # 火花强，1秒
        elif intensity > 0.3:
            return CONFIG["check_interval"]  # 火花中等，2秒
        else:
            return CONFIG["check_interval_max"]  # 火花弱，3秒
...
class SafetyTriggerSystem:
    def __init__(self, config, detector_weld, detector_cut):
        ...
        self.dynamic_interval = config["check_interval"]  # 初始使用默认间隔
    ...
    def _update_dynamic_interval(self, current_time):
        ...
        if current_time - self.last_interval_update >= 0.5:
            ...
            # 可选：打印动态间隔变化
            # if self.prev_scene == 'weld':
            #     intensity_level = self.spark_detector.get_intensity_level()
            #     print(f"[动态间隔] 火花强度: {intensity_level} ({self.current_spark_intensity:.2f}) -> 间隔: {self.dynamic_interval:.1f}秒")
    ...
    def _process_welding(self, frame, current_time, frame_count, has_smoke=False, smoke_score=0):
        ...
        if not should_trigger and self.prev_scene == 'weld' and not cache_valid and frames_since_last > 30:
            ...
            if (current_time - self.last_qwen_call_time) > self.dynamic_interval:
                ...
                print(
                    f"[动态间隔] 火花强度: {intensity_level} ({self.current_spark_intensity:.2f}) -> 触发间隔: {self.dynamic_interval:.1f}秒")
    ...
    while True:
        ...
        if current_scene == "weld" and trigger_system.current_spark_intensity > 0:
            ...
            spark_color = (0, 255, 0) if trigger_system.current_spark_intensity <= 0.3 else \
                (0, 255, 255) if trigger_system.current_spark_intensity <= 0.6 else \
                    (0, 0, 255)
...
def main():
    ...
    print(f"动态间隔范围: {CONFIG['check_interval_min']}s ~ {CONFIG['check_interval_max']}s")
    print("=" * 50)
